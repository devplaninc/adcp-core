...
func cmdEntry(cmd string) *adcp.PrefetchEntry {
	return adcp.PrefetchEntry_builder{Cmd: strPtr(cmd)}.Build()
}

func prefetchWith(entries ...*adcp.PrefetchEntry) *adcp.Prefetch {
	return adcp.Prefetch_builder{Entries: entries}.Build()
}

func assertResult(t *testing.T, result map[string]*adcp.FetchedData, expected map[string]string) {
	t.Helper()
...
func TestProcessor_Process(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "nil entries",
			prefetch: adcp.Prefetch_builder{}.Build(),
		},
		{
			name:     "empty entries",
